...
- `failureThreshold: 0.001` — 0.1% кадра (около 900 пикселей). Ловит сдвиг отдельного блока, при этом выше уровня нестабильности рендера: повторный прогон в том же окружении даёт ноль расхождений.
...
Порог подобран под сравнение **Linux против Linux** (эталон из CI, прогон в CI). Локально на Windows тот же прогон против CI-эталонов даёт дельты до 4.6% на текстоёмких стори — это не регрессии, а разные растеризаторы шрифтов, и никакой разумный порог их не покроет. Практический вывод: **локальный `yarn test:ui` под Windows/macOS не является критерием** — красный результат там ничего не доказывает, смотри на CI. Подробнее ниже, в разделе про регенерацию baseline.
